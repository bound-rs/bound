...
| Excessive allocation | The manifest is limited to 64 MiB, and each list's length is checked against its limit before any element is decoded (1,000,000 resources and blobs, 100,000 arguments and environment bindings, 100,000 entries in all list variables together), which bounds the memory a crafted manifest can make a reader use. Contents are streamed through fixed buffers, never loaded whole. |
...
| A list variable split into more entries (`PATH=bin:evil` from one entry) | A list entry is a whole path or literal: literal entries and bundled names in list variables may not contain the platform's list separator (`:`, or `;` on Windows), checked when building and when validating any manifest. Only the bundle root, from the temporary directory, could bring one at run time, and the launcher then refuses to start. |
| Bundled programs shadowing system ones | A bundled directory prepended to `PATH` is searched before the caller's directories, by the launcher and by the program alike: that is the point of the option. `inspect` shows every list variable with its entries in order. |
